...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if do_eval:
            ev = evaluate(model, test_dl, device, args.eval_steps)
            hist["test"].append({"epoch": epoch, **ev})
            cur_rmse = ev["rmse_avg"]
            print(f"  Epoch {epoch:03d}/{args.epochs} | loss={train_loss:.4g} | "
                  f"Te_mae={ev['mae_Te']:.4f} Ti_mae={ev['mae_Ti']:.4f} | "
                  f"rmse={ev['rmse_avg']:.4f}")
        else:
            cur_rmse = float("inf")
            if epoch % 10 == 0:
                print(f"  Epoch {epoch:03d}/{args.epochs} | loss={train_loss:.4g}")

        ckpt = dict(arch="fm_unet_te_ti", y_channels=Y_CHANNELS,
