...
def main():
    ...
    y_channels = ckpt["y_channels"]  # e.g. [0], [1], or [0,1]
    ch_names = ckpt.get("ch_names", [ALL_Y_NAMES[c] for c in y_channels])

    model = VelocityUNet(c_in, c_out, base, t_dim).to(device)
    model.load_state_dict(ckpt["model_state"])
    model.eval()

    # ── Load norm stats (selected channels only) ──
    ...
    y_mean_sel = y_mean_full[y_channels]
    y_std_sel  = y_std_full[y_channels]

    print(f"\n{'='*60}")
    print(f"Inference — {', '.join(ch_names)}")
    print(f"  y_channels: {y_channels}")
    for i, name in enumerate(ch_names):
        print(f"  y_mean[{name}]={y_mean_sel[i]:.4f}  y_std[{name}]={y_std_sel[i]:.4f}")
    ...
    for view in range(3):
        ...
        Y_gt_norm = np.array(Y_gt_full[:, y_channels, :, :], dtype=np.float32)  # (N, c_out, H, W)
        ...
        vm = {}
        for i, name in enumerate(ch_names):
            vm[f"mae_eV_{name}"]   = float(mae_eV[i])
            vm[f"rmse_eV_{name}"]  = float(rmse_eV[i])
            vm[f"mae_norm_{name}"] = float(mae_norm[i])
            vm[f"rmse_norm_{name}"]= float(rmse_norm[i])
        vm["mae_eV_avg"]   = float(mae_eV.mean())
        vm["rmse_eV_avg"]  = float(rmse_eV.mean())
        vm["mae_norm_avg"] = float(mae_norm.mean())
        vm["rmse_norm_avg"]= float(rmse_norm.mean())
        all_metrics[f"view{view}"] = vm

        norm_str = "  ".join(f"{n}={vm[f'mae_norm_{n}']:.4f}" for n in ch_names)
        eV_str   = "  ".join(f"{n}={vm[f'mae_eV_{n}']:.2f}" for n in ch_names)
        rmse_str = "  ".join(f"{n}={vm[f'rmse_eV_{n}']:.2f}" for n in ch_names)
        print(f"  Normalized MAE:  {norm_str}")
        print(f"  Physical MAE:    {eV_str} eV")
        print(f"  Physical RMSE:   {rmse_str} eV")
