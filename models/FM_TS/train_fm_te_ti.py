...
ALL_Y_NAMES = ["Te","Ti"] + [f"na_{s}" for s in SPECIES] + [f"ua_{s}" for s in SPECIES]


# ═══════════════════════════════════════════════════════════════════════════
# Dataset — selects specified channels from Y
# ═══════════════════════════════════════════════════════════════════════════

class ViewDataset(Dataset):
    def __init__(self, x_path: Path, y_path: Path, y_channels: List[int]):
    ...
    def __getitem__(self, idx):
        ...
        y = torch.from_numpy(y_full[self.y_channels])  # (c_out, H, W)
        m = (x[0:1] > 0.5).float()
        return x, y, m
...
@torch.no_grad()
def evaluate(model, loader, device, steps=50, ch_names=None):
    model.eval()
    ...
    result = {"mae_avg": float(mae_sum.mean()), "rmse_avg": float(rmse_sum.mean())}
    if ch_names:
        for i, name in enumerate(ch_names):
            result[f"mae_{name}"] = float(mae_sum[i])
            result[f"rmse_{name}"] = float(rmse_sum[i])
    return result
...
def main():
    ...
    ap.add_argument("--y_channels", default="0,1",
                    help='Which Y channels to predict: "0" for Te, "1" for Ti, "0,1" for both')
    ...
    # Parse y_channels
    y_channels = [int(c.strip()) for c in args.y_channels.split(",") if c.strip()]
    ch_names = [ALL_Y_NAMES[c] for c in y_channels]
    ...
    def load_ds(split, view):
        ...
        return ViewDataset(xp, yp, y_channels)
    ...
    x0, y0, m0 = train_ds[0]
    c_in  = x0.shape[0]   # 9
    c_out = y0.shape[0]   # len(y_channels)

    print(f"\n{'='*60}")
    print(f"Flow Matching — {', '.join(ch_names)}")
    print(f"  Train: {len(train_ds)} samples  |  Test: {len(test_ds)} samples")
    print(f"  c_in={c_in}  c_out={c_out}  canvas={x0.shape[1]}x{x0.shape[2]}")
    print(f"  Y channels: {y_channels} ({', '.join(ch_names)})")
    print(f"{'='*60}")
    ...
    config = dict(arch="fm_unet", y_channels=y_channels, ch_names=ch_names,
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if do_eval:
            ev = evaluate(model, test_dl, device, args.eval_steps, ch_names)
            hist["test"].append({"epoch": epoch, **ev})
            cur_rmse = ev["rmse_avg"]
            per_ch = "  ".join(f"{n}_mae={ev.get(f'mae_{n}', 0):.4f}" for n in ch_names)
            print(f"  Epoch {epoch:03d}/{args.epochs} | loss={train_loss:.4g} | "
                  f"{per_ch} | rmse={ev['rmse_avg']:.4f}")
        else:
            cur_rmse = float("inf")
            if epoch % 10 == 0:
                print(f"  Epoch {epoch:03d}/{args.epochs} | loss={train_loss:.4g}")

        ckpt = dict(arch="fm_unet", y_channels=y_channels, ch_names=ch_names,
