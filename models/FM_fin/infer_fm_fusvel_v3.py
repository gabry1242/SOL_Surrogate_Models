#!/usr/bin/env python3
"""
infer_fm_fusvel_v3.py
─────────────────────────────────────────────────────────────────────────────
Inference for VelocityUNet trained by train_fm_fusvel_full_v3.py.

Supports two modes (auto-detected from checkpoint, or overridden via CLI):
  --mode cfm      ODE integration from noise → prediction  (default for CFM)
  --mode direct   Single forward pass with y_t=0, t=1      (direct regression)

Usage
─────
# Direct regression checkpoint
python infer_fm_fusvel_v3.py \
    --checkpoint scripts/runs/fusvel_v3_direct/checkpoint_best.pt \
    --test_prefix scripts/tensor/3views_4d/test/global3v \
    --test_split  test \
    --out_dir     scripts/runs/fusvel_v3_direct/infer_test \
    --batch_size  32

# OT-CFM checkpoint (with ODE integration)
python infer_fm_fusvel_v3.py \
    --checkpoint scripts/runs/fusvel_v3_cfm/checkpoint_best.pt \
    --test_prefix scripts/tensor/3views_4d/test/global3v \
    --test_split  test \
    --out_dir     scripts/runs/fusvel_v3_cfm/infer_test \
    --batch_size  32 \
    --ode_steps   50 \
    --n_samples   1
"""
...
# ─────────────────────────────────────────────────────────────────────────────
# ODE integration (Euler method)  — used for CFM mode
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# Direct regression forward  (single pass, no ODE)  — used for direct mode
# ─────────────────────────────────────────────────────────────────────────────
...
def direct_forward(
    ...
    """
    Single forward pass: model(x, zeros, t=1, view).
    Returns predictions in normalised transformed space.
    """
    B, _, H, W = x_cond.shape
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Infer with a VelocityUNet checkpoint (direct or OT-CFM)."
    )
    ...
    ap.add_argument("--ode_steps",    type=int, default=50,
                    help="Number of Euler integration steps (CFM mode only)")
    ap.add_argument("--n_samples",    type=int, default=1,
                    help="Number of samples per input (CFM mode only, >1 for uncertainty)")
    ap.add_argument("--mode",         default=None, choices=["cfm", "direct"],
    ...
    # Auto-detect mode from checkpoint, allow CLI override
    ...
    # In direct mode, multi-sample makes no sense (deterministic output)
    n_samples = 1 if is_direct else args.n_samples

    print(f"\n{'='*70}")
    print(f"  VelocityUNet Inference — {method}")
    print(f"  Mode       : {'DIRECT regression (single forward pass)' if is_direct else 'OT-CFM (Euler ODE integration)'}")
                ...
                for x, m in dl:
                    ...
                    if is_direct:
                        ...
                        pred = euler_integrate(
                            model, x, m, view_ids,
                            c_out=c_out, n_steps=args.ode_steps,
                            device=device,
                        )
