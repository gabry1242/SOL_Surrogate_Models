...
Supports three modes (auto-detected from checkpoint, or overridden via CLI):
  ...
  --mode xpred    Data-prediction ODE integration:
                    v = (ŷ₁ - y_t) / (1 - t)
  --mode direct   Single forward pass with y_t=0, t=1

Usage
─────
# x-prediction flow matching checkpoint
python infer_fm_fusvel_v4.py \\
    ...
    --test_split  test \\
    --out_dir     scripts/runs/fusvel_v4_xpred/infer_test \\
    --batch_size  32 \\
    --ode_steps   50
"""
...
# ─────────────────────────────────────────────────────────────────────────────
# ODE integration: velocity-prediction  (--mode cfm)
# ─────────────────────────────────────────────────────────────────────────────
...
def euler_integrate_velocity(
    ...
    """Model outputs velocity v_θ directly. Euler: y_{t+dt} = y_t + v_θ * dt."""
    B, _, H, W = x_cond.shape
    dt = 1.0 / n_steps
...
# ─────────────────────────────────────────────────────────────────────────────
# ODE integration: data-prediction  (--mode xpred)
# ─────────────────────────────────────────────────────────────────────────────
#
# The model predicts ŷ₁ = f_θ(x, y_t, t).
# The velocity is derived as:
#     v = (ŷ₁ - y_t) / (1 - t)
#
# Euler step:
#     y_{t+dt} = y_t + v * dt
...
#              = y_t + (ŷ₁ - y_t) * dt / (1 - t)
#
# Near t → 1, (1 - t) becomes small, so we clamp it for stability.
# In practice the last step's ŷ₁ is already a good prediction,
# so we can just use it directly for the final output.
#

def euler_integrate_xpred(
    model: VelocityUNet,
    x_cond: torch.Tensor,
    mask: torch.Tensor,
    view_id: torch.Tensor,
    ...
    """
    Model outputs ŷ₁ (data prediction). Velocity derived as (ŷ₁ - y_t)/(1-t).
    Returns the final y_t after integration (≈ ŷ₁ from the last step).
    """
    B, _, H, W = x_cond.shape
    dt = 1.0 / n_steps
    y_t = torch.randn(B, c_out, H, W, device=device) * mask
    ...
    for step in range(n_steps):
        ...
        # Model predicts clean data ŷ₁
        y1_pred = model(x_cond, y_t, t, view_id)

        # Derive velocity: v = (ŷ₁ - y_t) / (1 - t)
...
def main() -> None:
    ...
    ap.add_argument("--ode_steps",    type=int, default=50,
                    help="Number of Euler integration steps (cfm/xpred modes only)")
    ap.add_argument("--n_samples",    type=int, default=1,
                    help="Number of samples per input (cfm/xpred modes only, >1 for uncertainty)")
    ...
    method = ckpt.get("method", "OT-CFM-velocity")
    ...
    if mode == "xpred":
        print(f" (data-prediction ODE)")
    elif mode == "cfm":
        print(f" (velocity-prediction ODE)")
    else:
