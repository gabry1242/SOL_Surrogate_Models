...
def inverse_transform_y(y_t, y_indices, pos_channels, signed_channels, eps, s_c):
    ...
    for j, orig_c in enumerate(y_indices):
        if int(orig_c) in pos_set:
            ...
            clamped = np.clip(y_t[:, j].astype(np.float64), -MAX_SYMLOG, MAX_SYMLOG)
            # inverse symlog: sign(t) * s_c * (10^|t| - 1)
            out[:, j] = (np.sign(clamped) * float(s_c[j]) *
                         (10.0 ** np.abs(clamped) - 1.0)).astype(np.float32)
        else:
