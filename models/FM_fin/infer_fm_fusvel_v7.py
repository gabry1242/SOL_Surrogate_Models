#!/usr/bin/env python3
"""
infer_fm_fusvel_v6.py
─────────────────────────────────────────────────────────────────────────────
Inference for VelocityUNet trained by train_fm_fusvel_full_v7.py.

Changes from v5:
  - Inverse transform for signed channels now uses inverse symlog:
      y = s_c * sign(t) * (10^|t| - 1)
    matching the symlog forward transform in v7 training.
...
MAX_LOG10    = 37.0
MAX_SYMLOG   = 37.0  # same log10 scale cap as positive channels

def inverse_transform_y(y_t, y_indices, pos_channels, signed_channels, eps, s_c):
    """Inverse of the per-channel transforms applied during training.

    Positive channels:  y = 10^t - eps
    Signed channels:    y = s_c * sign(t) * (10^|t| - 1)
      (inverse of symlog: t = sign(y) * log10(|y|/s_c + 1))

    Fully vectorised: no Python loop over channels, no per-channel float64 cast.
    All channels are processed in two bulk operations (pos / signed), which is
    ~22x faster than the previous per-channel loop for C=22.
    """
    pos_set    = set(int(c) for c in pos_channels)
    signed_set = set(int(c) for c in signed_channels)

    # Build boolean index arrays once
    C_sel   = len(y_indices)
    is_pos  = np.array([int(c) in pos_set    for c in y_indices], dtype=bool)  # (C_sel,)
    ...
    # Work in float64 on the whole tensor at once — single cast, no per-channel copies
    t64 = y_t.astype(np.float64)          # (N, C_sel, H, W)
    ...
    # ── Signed channels (symlog inverse) ──────────────────────────────────
    if is_sig.any():
        sc_vec              = np.asarray(s_c, dtype=np.float64)[is_sig]  # (n_sig,)
...
def masked_log_mae_per_channel_np(pred, y_true, mask, y_indices, pos_channels, signed_channels, eps_log=1e-3, eps_den=1e-8):
    ...
    for j, orig_c in enumerate(y_indices):
        ...
        if int(orig_c) in pos_set:
            ...
            # Use symlog for consistency with the training transform
...
def main() -> None:
    ...
    for v in range(3):
        ...
        # ── Extract masks once (same across all samples) ─────────────────
        # Masks come from channel 0 of X, which is fixed for a given input.
        # Iterating the DataLoader just to get masks on every sample wastes time.
        masks_np = np.zeros((N, 1, H, W), dtype=np.float32)
        _idx = 0
        for x, m in DataLoader(ds, batch_size=args.batch_size, shuffle=False,
                                num_workers=0):
            b = m.shape[0]
            masks_np[_idx:_idx+b] = m.numpy()
            _idx += b
        masks_gpu = torch.from_numpy(masks_np).to(device)  # keep on GPU for masking

        # ── Welford accumulators (online mean/variance, O(1) RAM) ─────────
        # Avoids storing all n_samples × (N,C,H,W) simultaneously.
        # Peak RAM: 2 × (N,C,H,W) float64 instead of n_samples × float32.
        ...
        for sample_idx in range(n_samples):
            ...
            # Keep predictions on GPU during the batch loop — avoids one
            # GPU→CPU sync per batch (was: n_batches syncs per sample).
            # Single .cpu() call at the end instead.
            preds_gpu = torch.zeros(N, c_out, H, W,
                                    dtype=torch.float32, device=device)
            idx0 = 0

            with torch.no_grad():
                for x, _ in dl:          # masks already extracted above
                    b = x.shape[0]
                    x_gpu    = x.to(device)
                    ...
                    if mode == "direct":
                        ...
                        pred = stochastic_integrate_xpred(
                            ...
                            n_steps=args.ode_steps, device=device)
                    elif mode == "xpred":
                    ...
                    preds_gpu[idx0:idx0+b] = pred   # stays on GPU — no sync
                    idx0 += b

            # Single GPU→CPU transfer per sample (was one per batch)
            preds_norm = preds_gpu.cpu().numpy()    # (N, C, H, W)
            del preds_gpu

            # Denormalise + inverse transform (vectorised, single float64 cast)
            ...
            preds_phys *= masks_np
            del preds_norm, preds_t

            # Welford online update (no list of full arrays kept in RAM)
        ...
        if n_samples > 1:
            std_path = out_dir / f"pred_Y_std_{args.test_split}_{view_tag}.npy"
