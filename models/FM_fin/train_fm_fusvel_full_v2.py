...
─────────────────────────────────────────────────────────────────────────────
REVISED full-dataset training with VelocityUNet — now using proper
Optimal Transport Conditional Flow Matching (OT-CFM) instead of degenerate
t=1 regression.

Changes from v1 (see FIXES below):
  [FIX-1]  Masking: remove redundant pred*mask; let loss handle masking
  [FIX-2]  Mask consistency: validated — no code change needed
  [FIX-3]  Channel imbalance: add Huber loss option for robustness
  [FIX-4]  Degenerate time: implement proper OT-CFM flow matching
  [FIX-5]  View conditioning: add learnable view embedding channel
  [FIX-6]  Normalization: keep cross-view (justified for shared weights)
  [FIX-7]  Learning rate: lower default to 3e-4
  [FIX-8]  Validation: add 10% hold-out validation split with monitoring

New CLI:
  --dataset <int>   percentage of training data (0–100), default 100

Usage
─────
python train_fm_fusvel_full_v2.py \\
    ...
    --lr            3e-4 \\
    --y_channels    all \\
    --save_dir      scripts/runs/fusvel_full_v2 \\
    --dataset       100
...
def main() -> None:
    ...
    print(f"\n{'='*70}")
    print(f"  Full-dataset training — VelocityUNet × 3 views (OT-CFM)")
    ...
    config = {
        ...
        "method":          "OT-CFM",  # [FIX-4] proper flow matching
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch_x, batch_y, batch_m, batch_v in train_dl:
            ...
            # [FIX-4] Proper flow matching training step
