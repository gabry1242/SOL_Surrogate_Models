#!/usr/bin/env python3
"""
train_fm_fusvel_full_v3.py
─────────────────────────────────────────────────────────────────────────────
Training with VelocityUNet — supports both OT-CFM flow matching and
direct regression (diagnostic baseline).

  --mode cfm      OT-CFM flow matching (default, same as v2)
  --mode direct   Direct regression: model predicts y_1 from (x, zeros, t=1).
                  Use this to verify the architecture & data pipeline before
                  investing in flow matching.

Usage
─────
# Flow matching (default)
python train_fm_fusvel_full_v3.py \\
    --tensor_prefix scripts/tensor/3views_4d/train/global3v \\
    --split train --epochs 200 --batch_size 16 --base 32 --lr 3e-4 \\
    --save_dir scripts/runs/fusvel_v3_cfm
...
python train_fm_fusvel_full_v3.py \\
    ...
    --split train --epochs 200 --batch_size 16 --base 32 --lr 3e-4 \\
    --mode direct \\
    --save_dir scripts/runs/fusvel_v3_direct
...
class MultiViewDataset(Dataset):
    ...
    [FIX-5] Now also returns a view_id integer so the model can condition
    on which view it is processing.
    """
    ...
    def __getitem__(self, idx: int):
        ...
        return (
            ...
            torch.tensor(view, dtype=torch.long),  # [FIX-5] view ID
        )
...
class SinusoidalTimeEmb(nn.Module):
    """[FIX-4] Now receives meaningful t ~ U[0,1] during training,
    not a degenerate constant t=1."""
...
class ConvBlock(nn.Module):
    """Conv block with GroupNorm for training stability."""
    def __init__(self, ci, co):
        super().__init__()
        self.net = nn.Sequential(
            nn.Conv2d(ci, co, 3, padding=1),
            nn.GroupNorm(min(8, co), co),  # GroupNorm for stability
            nn.GELU(),
...
class FiLMBlock(nn.Module):
    """[FIX-4] FiLM conditioning is now meaningful because t varies.
    Also added GroupNorm."""
    ...
    def forward(self, x, t_emb):
        ...
        # Apply FiLM: scale and shift, then activate
...
class VelocityUNet(nn.Module):
    """
    FiLM-conditioned dual-encoder UNet for flow matching.

    [FIX-4] forward(x_cond, y_t, t) now receives meaningful inputs:
      - y_t is the noisy interpolant at time t (not zeros)
      - t is sampled from U[0,1] (not always 1)
      - The model predicts the velocity field v_θ(x, y_t, t)

    [FIX-5] Added view embedding: a learned embedding per view that is
      broadcast spatially and concatenated to the conditioning input.
    """
    def __init__(
        self,
        c_in: int,
        ...
        # [FIX-5] View embedding: maps view_id → spatial broadcast channel
        self.view_emb_dim = 4  # compact embedding
        self.view_emb = nn.Embedding(n_views, self.view_emb_dim)

        self.time_emb = SinusoidalTimeEmb(t_dim)

        # [FIX-5] CondEncoder input includes view embedding channels
        self.cond_enc = CondEncoder(c_in + self.view_emb_dim, base)

        # Main encoder: x_cond + view_emb + y_t
    ...
    def forward(
        ...
        y_t:    torch.Tensor,    # (B, c_out, H, W)  — noisy interpolant
        t:      torch.Tensor,    # (B,)              — flow time ∈ [0,1]
        view_id: torch.Tensor,   # (B,)              — view index 0/1/2
    ) -> torch.Tensor:
        te = self.time_emb(t)

        # [FIX-5] Broadcast view embedding spatially and concat to x_cond
        B, _, H, W = x_cond.shape
        v_emb = self.view_emb(view_id)                         # (B, view_emb_dim)
        v_emb_spatial = v_emb[:, :, None, None].expand(B, -1, H, W)  # (B, view_emb_dim, H, W)
        x_with_view = torch.cat([x_cond, v_emb_spatial], dim=1)      # (B, c_in + view_emb_dim, H, W)

        c1, c2, c3 = self.cond_enc(x_with_view)

        # [FIX-4] y_t now carries information (the noisy interpolant)
...
# ─────────────────────────────────────────────────────────────────────────────
# [FIX-4] Flow Matching training step
# ─────────────────────────────────────────────────────────────────────────────
#
# OT Conditional Flow Matching (Lipman et al., 2023):
#   - Sample noise:    y_0 ~ N(0, I) * mask
#   - Sample time:     t   ~ U[0, 1]
#   - Interpolant:     y_t = (1 - t) * y_0 + t * y_1     (OT path)
#   - Target velocity: u_t = y_1 - y_0                   (constant along OT path)
#   - Model predicts:  v_θ(x, y_t, t)
#   - Loss:            masked_MAE(v_θ, u_t)
#
# At inference, we integrate from y_0 ~ N(0,I) to y_1 using Euler steps.
# This is fundamentally different from the degenerate t=1 regression:
#   - t varies → FiLM conditioning learns time-dependent features
#   - y_t varies → dual encoder sees meaningful intermediate states
#   - Multiple samples possible → uncertainty quantification
#

def flow_matching_step(
    model: VelocityUNet,
    x: torch.Tensor,          # (B, c_in, H, W) normalised conditioning
    y_1: torch.Tensor,        # (B, c_out, H, W) normalised target
    ...
    Returns (loss, per_channel_mae) for logging.
    """
    B, C_out, H, W = y_1.shape
    device = x.device

    # Sample noise y_0 ~ N(0, I), masked to active region
    y_0 = torch.randn_like(y_1) * mask

    # Sample flow time t ~ U[eps, 1-eps] (avoid boundary instabilities)
    t = torch.rand(B, device=device) * 0.98 + 0.01  # t ∈ [0.01, 0.99]

    # OT interpolant: y_t = (1-t)*y_0 + t*y_1
    t_expand = t[:, None, None, None]
    y_t = (1 - t_expand) * y_0 + t_expand * y_1

    # Target velocity: u_t = y_1 - y_0  (constant along the OT path)
    u_t = y_1 - y_0

    # Model prediction
    v_pred = model(x, y_t, t, view_id)

    # [FIX-1] Masking is applied ONLY in the loss, not on the prediction.
    # This avoids zeroing gradients in masked regions unnecessarily and
    # keeps the loss computation clean.
    if loss_fn == "huber":
...
# ─────────────────────────────────────────────────────────────────────────────
# Direct regression training step  (diagnostic baseline)
# ─────────────────────────────────────────────────────────────────────────────
#
# Same model, but used as a plain regressor:
#   - y_t  = zeros   (no noise input)
#   - t    = 1       (fixed)
#   - pred = model(x, zeros, 1, view)
#   - loss = masked_MAE(pred, y_1)
#
# This tests whether the architecture + normalisation can reconstruct the
# target fields at all.  If this fails, flow matching has no hope either.
#
...
def direct_regression_step(
    ...
    Returns (loss, per_channel_mae).
    """
    B, C_out, H, W = y_1.shape
    device = x.device

    # No noise, no interpolation — just zeros as the "state" input
...
def masked_mae_per_channel(pred, target, mask):
    """[FIX-1] Loss handles masking — no need to pre-mask predictions."""
    if mask.shape[1] == 1:
...
def masked_huber_per_channel(pred, target, mask, delta=1.0):
    """[FIX-3] Huber loss for robustness to outlier channels (ua)."""
    if mask.shape[1] == 1:
        mask = mask.expand_as(pred)
...
def main() -> None:
    ...
    # [FIX-7] Default learning rate lowered from 1e-3 to 3e-4
    ...
    print(f"  Mode   : {args.mode.upper()} {'(OT-CFM flow matching)' if args.mode == 'cfm' else '(direct regression baseline)'}")
    ...
    # We subsample SIMULATIONS, not individual (sim, view) pairs.
    # This preserves the 3-view structure: if a simulation is selected,
    # all 3 of its views are included.
    ...
    # Important: stats are always computed on the full dataset, not the
    # subsampled one, to ensure consistent normalization across experiments.
    ...
    # Map selected simulation IDs to (sim, view) indices in the flat dataset
    # Dataset layout: idx = sim * n_views + view
    n_views = 3
    selected_flat_ids = []
    for sim_id in selected_sims:
        for v in range(n_views):
            selected_flat_ids.append(int(sim_id) * n_views + v)

    # [FIX-8] Split selected simulations into train/val at the simulation level
    n_val_sims = max(1, int(len(selected_sims) * args.val_frac))
    n_train_sims = len(selected_sims) - n_val_sims

    # Shuffle selected_sims deterministically for the split
    ...
    config = {
        ...
        "method":          "direct-regression" if args.mode == "direct" else "OT-CFM",
        "mode":            args.mode,
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch_x, batch_y, batch_m, batch_v in train_dl:
            ...
            batch_v = batch_v.to(device, non_blocking=True)  # [FIX-5] view IDs

            # Dispatch to correct training step based on mode
            if args.mode == "direct":
                loss, mae_c = direct_regression_step(
                    model, batch_x, batch_y, batch_m, batch_v,
                    loss_fn=args.loss_fn,
                )
            else:
                loss, mae_c = flow_matching_step(
                    model, batch_x, batch_y, batch_m, batch_v,
                    loss_fn=args.loss_fn,
                )
        ...
        # ─── Validate ────────────────────────────────────────────────────
        # [FIX-8] Validation loop to detect overfitting
        model.eval()
        val_loss = 0.0
        val_mae  = None
        n_val    = 0
        with torch.no_grad():
            for batch_x, batch_y, batch_m, batch_v in val_dl:
                ...
                if args.mode == "direct":
                    loss_v, mae_v = direct_regression_step(
                        model, batch_x, batch_y, batch_m, batch_v,
                        loss_fn=args.loss_fn,
                    )
                else:
                    loss_v, mae_v = flow_matching_step(
                        model, batch_x, batch_y, batch_m, batch_v,
                        loss_fn=args.loss_fn,
                    )
        ...
        # [FIX-8] Save best on VALIDATION loss (not training loss)
