...
def xpred_flow_matching_step(
    ...
    # Sample flow time with configurable schedule
    t = sample_t(B, t_schedule, device)
...
def direct_regression_step(
    ...
    view_id: torch.Tensor,
    loss_fn: str = "mae",
    t_schedule: str = "uniform",  # ignored, present for interface consistency
) -> Tuple[torch.Tensor, torch.Tensor]:
...
def main() -> None:
    ...
    ap.add_argument("--t_schedule",    default="low_biased",
                    choices=["uniform", "low_biased", "logit_normal"],
                    help="Time sampling schedule: uniform, low_biased (recommended for xpred), logit_normal")
