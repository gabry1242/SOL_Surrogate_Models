"""
train_fm_fusvel_full_v7.py
─────────────────────────────────────────────────────────────────────────────
Training with VelocityUNet — supports three modes:

Changes from v6:
  - Velocity channels now use symmetric log transform:
      symlog(y) = sign(y) * log10(|y| / s_c + 1)
    instead of arcsinh(y / s_c).  This compresses large magnitudes more
    aggressively while preserving the sign and providing
    a smooth transition around zero.

  --mode cfm      Velocity-prediction OT-CFM flow matching
  --mode xpred    Data-prediction OT-CFM flow matching (recommended)
  --mode direct   Direct regression: single pass, no flow (baseline)

Key feature: SELF-CONDITIONING (--self_cond_prob, default 0.5)
  With probability p, the training input y_t is constructed from the
  model's own prediction instead of ground truth.  This teaches the
  model to handle imperfect inputs — exactly what happens during
  multi-step Euler integration.  Without self-conditioning, the model
  only ever sees y_t on the clean interpolation path, creating a
  train/test mismatch that causes Euler divergence.

Usage
─────
# x-prediction with self-conditioning
python train_fm_fusvel_full_v6.py \\
    --tensor_prefix scripts/tensor/3views_4d/train/global3v \\
    --split train --epochs 200 --batch_size 16 --base 64 --lr 3e-4 \\
    --mode xpred --t_schedule uniform --self_cond_prob 0.5 \\
    --save_dir scripts/runs/fusvel_v6_xpred_selfcond
...
def _symlog(y: np.ndarray, s_c: float) -> np.ndarray:
    """Symmetric log10 transform:  sign(y) * log10(|y| / s_c + 1)."""
...
def compute_y_stats(y_arr, mask_arr, y_indices, eps=1e-3):
    ...
    Transforms (computed per channel):
      Positive channels (Te, Ti, na): log10(max(y, 0) + eps)
      Signed channels (ua):           sign(y) * log10(|y| / s_c + 1)
        where s_c is the per-channel masked std in physical units.
    """
...
# ─────────────────────────────────────────────────────────────────────────────
# Time sampling schedules
# ─────────────────────────────────────────────────────────────────────────────
#
# Uniform sampling spends equal time on all t values. But with x-prediction,
# high-t cases are trivially easy (y_t ≈ y_1), so the model learns to copy
# y_t rather than using x to predict y_1.  Biasing toward low t forces the
# model to learn the hard cases where it must rely on x_cond.
#
#   uniform:      t ~ U[0.01, 0.99]  (original, bad for xpred from scratch)
#   low_biased:   t = u²             (quadratic, concentrates near t=0)
#   logit_normal: t = σ(N(μ,σ²))     (concentrates at both extremes)
#

def sample_t(B: int, schedule: str, device: torch.device) -> torch.Tensor:
    """Sample flow time t ∈ [0.01, 0.99] with the given schedule."""
    if schedule == "uniform":
        return torch.rand(B, device=device) * 0.98 + 0.01

    elif schedule == "low_biased":
        # t = u^2 where u ~ U[0,1] → PDF(t) = 1/(2√t), concentrates near 0
        u = torch.rand(B, device=device)
        return (u ** 2) * 0.98 + 0.01

    elif schedule == "logit_normal":
        # t = sigmoid(N(0, 1)) → concentrates at both t≈0 and t≈1
        # Using mean=0, std=1 gives a moderate spread
        u = torch.randn(B, device=device)
        return torch.sigmoid(u) * 0.98 + 0.01
...
def flow_matching_step(
    ...
    self_cond_prob: float = 0.0,  # ignored for velocity prediction
...
# ─────────────────────────────────────────────────────────────────────────────
# Training step: x-prediction OT-CFM with SELF-CONDITIONING  (--mode xpred)
# ─────────────────────────────────────────────────────────────────────────────
#
# Data-prediction parameterization (Lipman et al., 2023):
#   - Probability path:  y_t = (1 - t) * y_0 + t * y_1
#   - Model predicts:    ŷ₁ = f_θ(x, y_t, t, view)
#   - Loss:              masked_MAE(ŷ₁, y_1)
#   - Velocity:          v = (ŷ₁ - y_t) / (1 - t)
#
# SELF-CONDITIONING (fixes multi-step Euler divergence):
#   The core problem: during training, y_t is always on the clean
#   interpolation path (built from ground truth y_1).  During Euler
#   inference, y_t is built from the model's OWN predictions, which
#   have errors.  The model was never trained on such inputs, so its
#   predictions become inconsistent and the trajectory diverges.
#
#   Fix: with probability p_self_cond, replace y_1 in the interpolant
#   with the model's own prediction ŷ₁.  This creates a "corrupted"
#   y_t that looks like what the model will actually see during Euler
#   integration.  The model learns to handle its own imperfections.
#
#   Concretely:
#     1. Sample t_early < t, construct y_{t_early} on the clean path
#     2. Run a DETACHED forward pass: ŷ₁ = model(x, y_{t_early}, t_early)
#     3. Build y_t using ŷ₁ instead of y_1:
#           y_t_self = (1 - t) * y_0 + t * ŷ₁
#     4. Train with gradient: loss = MAE(model(x, y_t_self, t), y_1)
#
#   The model is ALWAYS trained against ground truth y_1 — only the
#   INPUT y_t is corrupted.  This teaches the model to correct errors.
#
...
def xpred_flow_matching_step(
    ...
    """
    X-prediction flow matching with self-conditioning.

    With probability self_cond_prob, the input y_t is constructed from
    the model's own prediction (simulating Euler trajectory error).
    Otherwise, y_t is on the clean interpolation path.
    Always trains against ground truth y_1.
    """
    B, C_out, H, W = y_1.shape
    device = x.device
    ...
    # Sample the target time for this training step
    t = sample_t(B, t_schedule, device)
    t_expand = t[:, None, None, None]

    # Decide whether to use self-conditioning for this batch
    use_self_cond = (torch.rand(1).item() < self_cond_prob) and model.training

    if use_self_cond:
        # Step 1: Pick an earlier time t_early < t for each sample
        # t_early = t * u where u ~ U[0, 0.8] — always earlier than t
        t_early = t * torch.rand(B, device=device) * 0.8
        t_early_expand = t_early[:, None, None, None]

        # Step 2: Construct y at the earlier time (clean path)
        y_t_early = (1 - t_early_expand) * y_0 + t_early_expand * y_1

        # Step 3: Get model's prediction from this earlier point (NO gradient)
        with torch.no_grad():
            y1_hat = model(x, y_t_early, t_early, view_id)

        # Step 4: Build y_t using model's prediction instead of ground truth
        # This simulates what Euler integration would produce
        y_t = ((1 - t_expand) * y_0 + t_expand * y1_hat) * mask
    else:
        # Standard clean interpolation path
        y_t = (1 - t_expand) * y_0 + t_expand * y_1

    # Forward pass WITH gradient — predict y_1
    y1_pred = model(x, y_t, t, view_id)

    # Loss always against ground truth y_1
    if loss_fn == "huber":
...
def direct_regression_step(
    ...
    loss_fn: str = "mae",
    t_schedule: str = "uniform",
    self_cond_prob: float = 0.0,  # ignored for direct mode
) -> Tuple[torch.Tensor, torch.Tensor]:
...
def main() -> None:
    ...
    ap.add_argument("--t_schedule",    default="uniform",
                    ...
                    help="Time sampling schedule (default: uniform)")
    ap.add_argument("--self_cond_prob", type=float, default=0.5,
                    help="Probability of self-conditioning per batch (0=off, default 0.5)")
        ...
        with torch.no_grad():
            for batch_x, batch_y, batch_m, batch_v in val_dl:
                ...
                loss_v, mae_v = step_fn(
                    ...
                    loss_fn=args.loss_fn,
                    t_schedule=args.t_schedule,
                    self_cond_prob=0.0,  # no self-cond during validation
                )
