"""
train_fm_fusvel_full_v8.py  (corrected - v8.1)
──────────────────────────────────────────────────────────────────────────────
...
Fix — two complementary operations applied after every encoder ConvBlock:
  ...
  2. INTER-VIEW vertical reinjection  (across canvases, same simulation)
     ...
     feature map into the adjacent gap rows of the neighbouring view:
       ...
       view1-top-gap    ← view2-Bottom_center bottom k content rows
...
class LayoutInfo:
    ...
    def _build_cross_mappings(self) -> List[dict]:
        ...
        # ── view2 bottom ↔ view1 top-center [CORRECTED] ────────────────
        # In build_3view_tensors.py:
        ...
        if len(strips1) >= 3:
            ...
            add_pair(
                ...
                dst_is_top_for_dv=True,    # view1 receives into its TOP gap  [BUG FIX]
            )
...
def _compute_loss_mae(pred, target, mask, loss_fn):
    """Compute loss and per-channel MAE. Avoids double computation for MAE mode."""
    if loss_fn == "huber":
        ...
        mae_c   = ch_loss.detach()       # same computation -- reuse, no grad
...
def xpred_flow_matching_step_3v(
    ...
    use_self_cond = model.training and (random.random() < self_cond_prob)
