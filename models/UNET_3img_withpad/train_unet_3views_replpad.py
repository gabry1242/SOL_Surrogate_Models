...
Modification vs train_unet_3views.py
-------------------------------------
ConvBlock now uses **replicate padding** instead of zero padding.

  Standard Conv2d(padding=1) fills the 1-pixel border with zeros before
  each convolution.  For a masked image this means every active pixel on
  the mask boundary gets a contribution from zero-valued neighbours,
  systematically biasing its representation toward zero.

  With padding_mode="replicate" the border is filled by copying the
  nearest real pixel value, so boundary activations no longer receive
  artificial zero contributions.  The spatial size of every feature map
  is unchanged; no other part of the architecture is affected.
"""
...
class ConvBlock(nn.Module):
    """Two-layer conv block with replicate padding.

    padding_mode="replicate" fills the 1-pixel border by copying the nearest
    real value instead of zero.  This prevents the zero-padding artifact at
    the mask boundary where active pixels would otherwise receive contributions
    from artificial zeros.  Spatial dimensions are unchanged (padding=1 still
    keeps H×W constant), and no other part of the UNet is affected.
    """

    def __init__(self, c_in: int, c_out: int):
        super().__init__()
        # padding_mode="replicate" is incompatible with padding as an int in
        # older PyTorch versions when used inside nn.Sequential with ReLU, so
        # we use a small explicit forward instead of nn.Sequential.
