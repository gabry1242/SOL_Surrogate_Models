...
def main():
    ...
    # ── Configurable raw-data filenames ──────────────────────────────────────
    # Defaults match CreateHiddenDataset.ipynb output naming.
    # For the original SOLPS dataset use --use_tmp_suffix (appends _tmp).
    ap.add_argument("--x_file",         default=None,
                    help="Filename for X inputs (default: X_units.npy, or X_tmp.npy with --use_tmp_suffix)")
    ap.add_argument("--te_file",        default=None)
    ap.add_argument("--ti_file",        default=None)
    ap.add_argument("--na_file",        default=None)
    ap.add_argument("--ua_file",        default=None)
    ap.add_argument("--fnixap_file",    default=None)
    ap.add_argument("--use_tmp_suffix", action="store_true",
                    help="Use legacy *_tmp.npy filenames (original SOLPS dataset)")
    args = ap.parse_args()

    # Resolve filenames: --use_tmp_suffix selects the legacy names; individual
    # --*_file overrides take priority over both defaults.
    if args.use_tmp_suffix:
        _x_def, _te_def, _ti_def = "X_tmp.npy", "te_tmp.npy", "ti_tmp.npy"
        _na_def, _ua_def, _fn_def = "na_tmp.npy", "ua_tmp.npy", "fnixap_tmp.npy"
    else:
        _x_def, _te_def, _ti_def = "X_units.npy", "te.npy", "ti.npy"
        _na_def, _ua_def, _fn_def = "na.npy", "ua.npy", "fnixap.npy"

    args.x_file      = args.x_file      or _x_def
    args.te_file     = args.te_file     or _te_def
    args.ti_file     = args.ti_file     or _ti_def
    args.na_file     = args.na_file     or _na_def
    args.ua_file     = args.ua_file     or _ua_def
    args.fnixap_file = args.fnixap_file or _fn_def
    ...
    # ------------------------------------------------------------------ #
    X_raw  = np.load(split_dir / args.x_file)      # (N, n_x_feat)
    te_raw = np.load(split_dir / args.te_file)     # (N,104,50)
    ti_raw = np.load(split_dir / args.ti_file)
    na_raw = np.load(split_dir / args.na_file)     # (N,104,50,10)
    ua_raw = np.load(split_dir / args.ua_file)
    n_x_feat = int(X_raw.shape[1])                 # auto-detected (6 or 8)
    print(f"  Loaded {args.x_file}  shape={X_raw.shape}  → n_x_feat={n_x_feat}")
    fnixap = None
    if args.include_fnixap:
        fnixap = np.load(split_dir / args.fnixap_file).astype(np.float32)
    ...
    # 4.  Channel counts                                                   #
    # ------------------------------------------------------------------ #
    c_in = 1 + n_x_feat + (2 if args.include_geometry else 0)
    ...
    # Broadcast scalar channels (1..n_x_feat) with mask — same per simulation for the
    # mask shape, values vary per sample
    for kk in range(n_x_feat):
    ...
    if args.include_geometry:
        g_start = 1 + n_x_feat
        X0_img[:, g_start:g_start + 2] = geom0[None]
        X1_img[:, g_start:g_start + 2] = geom1[None]
        X2_img[:, g_start:g_start + 2] = geom2[None]
    ...
    # Meta
    in_ch_names = (["mask"] + [f"x{kk}" for kk in range(n_x_feat)] +
