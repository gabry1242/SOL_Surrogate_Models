...
class NpyTensorDataset(Dataset):
    def __init__(self, x_path: Path, y_path: Path, y_channels: Tuple[int, ...] | None = None):
        self.X = np.load(x_path)  # (N,C,H,W) float32
        self.Y = np.load(y_path)  # (N,C,H,W) float32
...
def parse_channels(s: str, c_out_total: int) -> Tuple[int, ...] | None:
    s = s.strip().lower()
    if s in ("all", "*"):
        return None
...
def evaluate(model: nn.Module, loader: DataLoader, device: torch.device) -> Dict[str, float]:
    ...
    for x, y, m in loader:
        ...
        rmse = masked_rmse(pred, y, m).item()
        mae_sum += mae
        rmse_sum += rmse
        n_batches += 1
    ...
    return {"mae": mae_sum / n_batches, "rmse": rmse_sum / n_batches}
...
def main() -> None:
    ...
    ds_train = NpyTensorDataset(train_x, train_y, y_channels=y_ch)
    ds_test = NpyTensorDataset(test_x, test_y, y_channels=y_ch)
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        for x, y, m in dl_train:
            ...
            with torch.no_grad():
                ...
                train_rmse_sum += float(rmse)
                n_batches += 1

        train_mae = train_mae_sum / max(1, n_batches)
